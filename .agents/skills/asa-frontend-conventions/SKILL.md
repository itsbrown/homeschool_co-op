...
- **Child profile Health tab looked read-only / Update did nothing** → `/children/:id` Health & Safety had no editor; Edit Profile required Gender even though school-code signup leaves it optional. Use **Add / Edit** on Health & Safety, or Edit Profile (gender optional). Allergies are `text`, not a chip list. Peanut-class alerts use `GET /api/parent/class-allergy-alerts` — never render classmate names from roster allergies. Membership agreement reminder is a **separate** Home banner (`MembershipAgreementBanner`, `GET /api/parent/agreements/status`) — no dismiss; it stays until the current version is signed. Do not lock login.
- **Parent Home Member ID is not self-serve** → do not add Edit/Save on My Membership. `PUT /api/parent/member-id` rejects a typed ID (`MEMBER_ID_NOT_SELF_ASSIGNABLE`) and the example `ASA-2025-X7K9M2`. Display + copy only; payment or staff assigns it.
...
- **`/parent/documents/:id` Unable to load document** → `DocumentDetailPage` custom-fetched `/api/membership-agreement/:id` (404). Signed snapshot is `GET /api/parent/documents/:id` with `agreementContent`. Use default fetcher and `queryKey: ['/api/parent/documents', id]`.
- **Toast import error** → imported `useToast` from Shadcn UI directory → import from `@/hooks/use-toast` instead
- **Roster birthday is a day early** → used `new Date("YYYY-MM-DD")` → use `RosterBirthday` / `formatBirthdayDisplay` (`shared/student-birthday.ts`)
...
- **School admin weekly print must match staff** → do not print the Week Planner editor chrome. Use `AsaWeeklySchedulePrintSheet` (`window.print()`). Mentor Schedule and Week Planner share that sheet. Print cells include the first description paragraph plus unique objectives (`printBlockSubtitles`) — title-only is not enough to see what is taught.
- **Week Planner cards looked like a timetable** → they used to clamp description to two lines and hide objectives/materials. Cards now preview the teaching script, objectives, and materials; **Lesson** opens `WeekPlanBlockDetailSheet`. Edit saves `materials` and `homework` as well as title/description.
...
- `client/src/components/roster/RosterBirthday.tsx` — class/attendance birthday + age (calendar date, no UTC shift)
...
- `client/src/lib/week-plan-lesson-content.ts` — description/objectives/materials preview + print subtitles
