...
- **Parents cannot find Important Documents** → nav used to say **My Documents**, Home listed only signed agreements, and `/parent/documents` opened the Agreements tab. Staff email that name. Label is **Important Documents**; default tab is School Documents (`?tab=school-documents`). Helper: `client/src/lib/parent-documents.ts`.
