...
| Client cache | `useEffectivePermissions` query key includes `activeRole` **with explicit `queryFn`** (default fetcher joins key segments into the URL); role switch invalidates `/api/me/effective-permissions` |
