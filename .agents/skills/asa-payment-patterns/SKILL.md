...
- **Amazon affiliate listings in the public store cart / Stripe** → any product with `affiliate_url` (Amazon or other vendor) is outbound-only. Snapshot/checkout must throw `StoreAffiliateCartError` (`400` `AFFILIATE_NOT_PURCHASABLE`). Never create a PaymentIntent for that line. CTA helper: `shared/store-product-cta.ts`. See `server/lib/store-pricing.ts` and `docs/APP_KNOWLEDGE/domains/public-store.md`.
- **Succeeded Stripe PI, no `payments` row** → client fulfill and/or webhook never ran. Post-payment verify only hooks the webhook. The **daily missed-PI sweep** lists succeeded ASA PIs vs `payments` and alerts `error_type=missed_payment_intent`. Ops: `node scripts/with-prod-env.mjs -- npx tsx server/scripts/sweep-missed-payment-intents.ts --days 90 --dry-run`. Auto-fix is off unless `--apply` / `MISSED_PI_SWEEP_AUTO_FIX`. Do not let `updateEnrollmentsWithPaymentIntent` reset `paymentStatus` to `pending` after class-pool apply.
- **Parent says they still owe but Payments shows $0 after a credited biweekly checkout** → `create-payment-intent` used to set `originalAmountCents` to the **full cart**. `totalCentsForBalanceAllocation` then used that gross for *this* PaymentIntent, so installment 1 wrote `total_paid = list`. Autopay skipped later dues (`effectiveBalance <= 0`) and cancelled them. **Never** stamp plan total on an installment PI — `originalAmountCents` = `PI.amount + credits` for that charge only, and allocation **caps** at that. Incident: Grace Mulcahy #66 Fall 2026 (Aug 20 `pi_3U6bUR`, leftover **$1,662.50**). Script: `fix-grace-mulcahy-fall-biweekly-ledger-production.ts`. Same pattern: Alanna Thomas #70, Amy Misso #91.
