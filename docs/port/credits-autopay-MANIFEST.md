...
1. **Migrations** — apply DDL:
   - `volunteer_credits` table (legacy ledger — keep for back-compat)
   - `credit_usage_logs` table (legacy log)
   - `credits` table (unified ledger)
   - `unified_credit_usage_logs` table
   - `credit_holds` table
   - *(Optional)* data-copy block that mirrors `volunteer_credits` rows into `credits` for back-compat
2. **Drizzle schema** — paste the table definitions + relations into `shared/schema.ts` (lines **3004–3199** in the **source** Replit repo). All columns: snake_case in DB, camelCase in Drizzle.
...
6. **API routes** — port `server/api/credits.ts` and `server/api/auto-pay.ts`; wire in `routes.ts` (`app.use("/api/credits", creditsRouter)` etc.).
...
8. **Bootstrap** — call `startAutoPayScheduler(...)` and `startCreditExpirationService(...)` from your app initializer (in source repo: `server/app-init.ts`). The scheduler ticks via `setInterval` (line ~889 of `auto-pay-scheduler.ts`).
...
The **`AUTO_APPLY_CREDITS`** flag has been removed in the source repo — credits are always applied. Do not re-introduce it in the receiving repo.

**Note:** This stack does **not** use Postgres RLS. Access control is enforced at the API layer (school-scoped via `req.user.schoolId`). If the receiving repo uses RLS (e.g. via Supabase), add policies separately.
...
| `server/init-db.ts` (credit blocks 1339–1620, plus credit_holds ~1589) | ~280 | Idempotent `CREATE TABLE IF NOT EXISTS` for all 5 credit tables + indexes + back-compat copy |
| `migrations/0000_gray_excalibur.sql` + `migrations/meta/{_journal.json,0000_snapshot.json}` | ~32k | Drizzle baseline migration + meta — needed only if you adopt drizzle-kit migrations rather than runtime DDL in `init-db.ts` |
| `server/storage.ts` (IStorage decls ~685–710 + delegation ~7910–7955) | ~70 | getAvailableCredits, useCredits, createCreditHolds, finalizeCreditHolds, releaseCreditHolds |
| `server/dbStorage.ts` (credit methods ~4508–5160) | ~650 | Storage implementations: FIFO logic, hold lifecycle, expiration filtering, unified_credit_usage_logs writes |
...
| Path | Purpose |
|------|---------|
| `.agents/skills/asa-credit-system/SKILL.md` | Authoritative ledger / FIFO / hold-lifecycle reference. Receiving Cursor repo **should** keep this |
...
### Probably skip (tangential — merge only if needed)
...
`server/api/enrollments.ts`, `server/api/financial-reports.ts`, `server/api/payment-history.ts`, `server/api/scheduled-payments.ts`, `server/api/stripe.ts`, `server/utils/cart-pricing.ts`, `docs/grace-mulcahy-backfill.md`

---

## 2. Schema definition line ranges (`shared/schema.ts` — **source** repo)
...
## 3. Webhook extraction guide (`server/webhook-handler.ts` — **source** ~2174 lines)

Do **not** port the whole file. Extract these blocks into your **existing** webhook:

| Branch | Lines (approx, source) | Purpose |
|--------|-------------------------|---------|
...
## 4. Re-creating this bundle (from **source** repo root)
...
## 5. Receiving repo checklist (this clone)

1. Download **`docs/port/credits-autopay-bundle-c27d976.tar.gz`** from Replit into this repo’s **`docs/port/`** (same folder as this file).
2. Verify SHA-256 if you recorded one for that tarball.
3. Extract to a scratch dir, diff against current tree, then merge file-by-file following **§0 Port order**.
4. Resolve conflicts: this repo already has `BillingPage`, `PaymentManagement`, `webhook-handler`, `scheduled-payment-reminders` — **merge credit branches**, do not blindly overwrite unrelated behavior.
5. Run migrations or `init-db` path per your production standard; then `npm run test:payments` / new integration tests from the bundle.
6. Open PR: **Port from Replit @ c27d976** with link to source SHA and note migration strategy chosen.
