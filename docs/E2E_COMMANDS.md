...
| [`e2e/school-admin-math-level-profile.spec.ts`](../e2e/school-admin-math-level-profile.spec.ts) | `npm run test:e2e -- e2e/school-admin-math-level-profile.spec.ts` | Student profile Math Level card: enter `3A`, see badge + history | `setup-progress-scenario` (`linkSupabaseAuthAdmin`) |
