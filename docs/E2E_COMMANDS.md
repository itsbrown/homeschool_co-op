...
| [`e2e/school-admin-week-planner-print.spec.ts`](../e2e/school-admin-week-planner-print.spec.ts) | `npm run test:e2e -- e2e/school-admin-week-planner-print.spec.ts` | Admin Week Planner **Print** uses the same ASA sheet as mentor Schedule | `setup-schedule-builder-scenario` (`linkSupabaseAuth`, `requireLinkedSeed`) |
...
| [`e2e/educator-weekly-schedule-plans.spec.ts`](../e2e/educator-weekly-schedule-plans.spec.ts) | `npm run test:e2e -- e2e/educator-weekly-schedule-plans.spec.ts` | Educator Schedule: published plan overlay + empty-day collapse toggle + print | `setup-schedule-builder-scenario` (`linkSupabaseAuth`) |
