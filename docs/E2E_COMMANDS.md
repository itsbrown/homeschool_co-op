...
| [`e2e/grade-placement-auto-place.spec.ts`](../e2e/grade-placement-auto-place.spec.ts) | `npm run test:e2e -- e2e/grade-placement-auto-place.spec.ts` | Auto-place roster badge + eligibility preview + **Place only** on Edit Class. `requireLinkedSeed`. | `setup-grade-placement-scenario` (`linkSupabaseAuth`) |
