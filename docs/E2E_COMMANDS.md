...
| [`e2e/parent-supply-list.spec.ts`](../e2e/parent-supply-list.spec.ts) | `npm run test:e2e -- e2e/parent-supply-list.spec.ts` | Sidebar/dashboard discoverability (filled + empty), household merge, Amazon CTA (no Add to cart), check persist, class-detail subset | `setup-supply-list-scenario` (`linkSupabaseAuthParent`); empty case `setup-public-store-scenario` (`withParent`, `linkSupabaseAuthParent`) |
