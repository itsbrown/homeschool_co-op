...
- **Store-ready classes:** `enrollmentOpen` + price (same gate as parent catalog); not legacy `isPublished`. `requireMemberId` programs are **not** store-ready — turn off Members only (member ID) before listing publicly.
...
- **Amazon affiliate products:** same `ImageUpload` / `storeProducts` cover upload as merch. `product_kind = 'affiliate'` (+ `affiliate_url`, `asin`, `affiliate_metadata`). Admin pastes an Associates URL → `POST …/affiliate/preview` → create product. Accepted: product `/dp/ASIN`, `amzn.to`, or search `amazon.com/s?k=…` **with ISBN-13/10 in the query** (ISBN-13 `978…` converts to ISBN-10 book ASIN). Search with no ISBN/ASIN returns `400` `ASIN_NOT_FOUND`. Preview sources: PA-API 5.0 when keys are set; non-prod mock; **manual** in production without keys (resolve short link → ASIN + Associates image widget; admin enters name/price). Image URL is the Associates ASIN widget (not `/images/I/{ASIN}`). Storefront CTA is **Buy on Amazon** when `affiliate_url` is an Amazon / `amzn.to` / `a.co` host (`rel="noopener noreferrer sponsored"`). Non-Amazon URLs (including an affiliate row pointing at Access Literacy) show **View product** (new tab, no `sponsored`). Never Add to cart / Stripe when `affiliate_url` is set. Broken covers overlay a Package icon on the `<img>` (do not unmount `src`). Migration `255-store-affiliate-products.sql`. Optional env for live title/price: `AMAZON_PAAPI_ACCESS_KEY`, `AMAZON_PAAPI_SECRET_KEY`, `AMAZON_PAAPI_PARTNER_TAG` (optional `AMAZON_PAAPI_MOCK=1`). Local cover upload without Replit Object Storage uses the E2E disk stub. Non-prod school-admin store routes auto-apply 251+255+261 via `ensurePublicStoreSchema`.
- **Pickup-only merch:** `store_products.pickup_only` (migration `261-store-product-pickup-only.sql`). Catalog exposes `pickupOnly`. Snapshot sets `pickupOnlyRequired` if any owned cart line is pickup-only; checkout then hides shipping and rejects `method=shipping` with `400 PICKUP_ONLY`. Mixed carts with a pickup-only item force pickup for the whole merch order. Non-prod `ensurePublicStoreSchema` applies 251+255+261.
- **Owned merch optional product link:** reuse `store_products.affiliate_url`. If set, families get **View product** (or **Buy on Amazon** when the host is Amazon) instead of Add to cart. Hint in create/edit: *If set, families open this URL instead of adding to cart.*
- **Edit listed products:** Public Store → Products **Listed products** cards have **Edit**. Dialog updates name, description (`Textarea`), display price, photo (`ImageUpload` / `storeProducts`), **List on public store**, and the outbound URL (**Affiliate link** required for affiliates; optional **Product link** for owned merch). `PATCH /api/school-admin/public-store/products/:id` keeps `affiliateUrl` for owned products (empty string → `null`). `/dp/ASIN` in a saved URL refreshes `asin` (no PA-API). Publish/hide uses school-scoped `upsertStoreListing` (do not use unscoped `PATCH /listings/:id`). GET `/products` returns `listingId` + `isPublished`. Affiliate **display** price is a snapshot; Amazon checkout is authoritative.
- **Delete product:** Edit dialog **Delete** → confirm. `DELETE /api/school-admin/public-store/products/:id` is school-scoped (404 if missing). Deletes the product listing, nulls `store_order_items.product_id` and `listing_id` (FKs have no `ON DELETE`; line `name` stays), then deletes the product. `supply_items.store_product_id` is `ON DELETE SET NULL`.
...
- **Detail** (`/store/:storeSlug/:itemSlug`): Same route for programs, sessions, and merch — layout adapts by `listingType` (stock/shipping copy for owned cart products; **Buy on Amazon** / **View product** for outbound URLs; schedule/enrollment copy for programs). Full description is never truncated. **Share** button builds a message with description + link (`?userId=` when logged in).
...
- **Affiliate / outbound cart guard:** Snapshot/checkout returns `400` + `AFFILIATE_NOT_PURCHASABLE` if any product with `affiliate_url` is submitted (Amazon or other vendor). Helper: `shared/store-product-cta.ts`.
...
- `PATCH /api/school-admin/public-store/products/:id` — name, description, `priceCents`, `imageUrl`, `affiliateUrl` (optional for owned; required for affiliate), optional `isPublished` (upserts product listing)
- `DELETE /api/school-admin/public-store/products/:id` — school-scoped; nulls order FKs; deletes listing + product
...
- **E2E:** `e2e/public-store-product-edit.spec.ts` — admin edit merch (fields, photo, hide/publish), affiliate URL + display price/photo, owned **View product** link, delete listing
