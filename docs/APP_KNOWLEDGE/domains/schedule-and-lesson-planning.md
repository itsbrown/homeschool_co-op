...
**Parent dashboard Upcoming Events:** KPI + card list school activities in the **next 7 days** (field trips, holidays, and other non-class rows from `GET /api/schedule`, plus `GET /api/calendar-events/parent/events`). Recurring class meetings (`type: "class"`) stay on `/schedule`. List shows at most 5 rows; KPI is the full 7-day count. Events after day 7 still appear on `/schedule`. Helper: `client/src/lib/parent-upcoming-events.ts`. Day sheet for school events shows type label, description, all-day or start–end, and venue (read-only).
...
| Home Upcoming Events is a list of daily classes | The 5-row preview included every enrolled class meeting, so field trips never reached the card | `mergeUpcomingEventsNext7Days` skips `type: "class"`. Class days stay on `/schedule` |
...
| Week Planner card shows only “1. Welcome & Greeting” or “Pre-K Group:” | `lessonTeachingPreview` used the first description line. Morning scripts put the activity on the next line. | Cards use `descriptionPreviewText` (up to six lines). Print and mentor Schedule still use `firstDescriptionParagraph`. **Lesson** opens the full script. |
