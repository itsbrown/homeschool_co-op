...
- **Parents cannot type a Member ID** — Home is display + copy. `PUT /api/parent/member-id` rejects a non-empty body (`MEMBER_ID_NOT_SELF_ASSIGNABLE`) and the docs example `ASA-2025-X7K9M2` (`RESERVED_EXAMPLE`). `hasMembership` follows `users.member_id`, so a typed example used to mark the household Active. IDs come from paid membership or staff. `isValidMemberIdFormat` rejects that reserved example exactly (year-locked).
...
| A restart changes paid, membership status, or pending installments with no new charge | `initializeDatabase` used to run ledger repairs on every boot (parent-wide credits, "full payment" membership stamp, comp installment cancel, Marek #389/#390, duplicate payment-history delete). | **Removed from boot 2026-09-21.** Do not put a balance or status UPDATE back in `init-db.ts`. Comp still cancels installments in `admin-enrollments.ts`. Membership still posts in `membership-fulfill-from-cart-intent.ts`. |
| A one-off comp script leaves pending installments | The comp button shrinks or cancels those rows. A script that only writes `comp_amount_cents` does not. | Do not restore the boot cancel (`remaining_balance = 0` is the wrong column). `processOneScheduledPayment` already skips and cancels when `effective_balance <= 0`. A partial comp can still charge the old installment amount. Share the comp-button installment sync, and cap the charge at `effective_balance`. |
| Parent cannot pay; cart empty; kids already on the roster | Parent unenroll / cart-clear **hard-deletes** `pending_payment` session seats. No row left. | Re-add the v2 session seat. After 2026-09-20, `audit_logs.action_type = parent_cart_remove_enrollment` (or `admin_delete_enrollment`) has the snapshot. |
