...
| Campus dropdown selection does not stick | Radix Select dismiss races with AlertDialog open (dialog closes immediately; PATCH never runs) | Defer confirm open (`setTimeout(0)`); bind Select to pending; plain Button for **Move family**; E2E `parent-profile-campus-change.spec.ts` |
