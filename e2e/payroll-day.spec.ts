...
test.describe("daily hours school feature", () => {
  test("super admin turns it on, then a granted parent saves a day", async ({ page, request }) => {
    ...
    await expect(page.getByTestId("payroll-day-list")).toBeVisible();
    await expect(page.getByText("Hourly rate")).toHaveCount(0);
    const focusRow = page.locator("[data-testid^='payroll-day-row-']").filter({ hasText: "Next up" });
    await expect(focusRow).toBeVisible({ timeout: 30_000 });
    const focusDate = (await focusRow.getAttribute("data-testid"))?.replace("payroll-day-row-", "") ?? "";
    expect(focusDate).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    await focusRow.getByTestId(`payroll-day-edit-${focusDate}`).click();
    await expect(page.getByTestId("payroll-day-edit-title")).toBeVisible({ timeout: 30_000 });
    ...
    await expect(page.getByTestId(`payroll-day-status-${focusDate}`)).toHaveText("Approved", { timeout: 30_000 });
    await page.getByTestId(`payroll-day-edit-${focusDate}`).click();
    await expect(page.locator("[data-testid^='away-']").first()).toHaveClass(/bg-primary/, { timeout: 30_000 });
    await page.getByRole("button", { name: "Different hours" }).click();
