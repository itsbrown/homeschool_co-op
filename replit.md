...
- **Multi-Role extractSchoolId Fix (PRODUCTION-READY)**: Fixed critical bug where school admin operations failed for multi-role users with "School not found" errors. extractSchoolId now uses active role's schoolId from user_roles table instead of user.schoolId for multi-role users, with fallback to legacy user.schoolId for single-role compatibility.
