...
## Recent Changes
### November 21, 2025
- **URGENT: Production-Safe extractSchoolId Fix (PRODUCTION-READY)**: Fixed zero metrics dashboard issue by reordering extractSchoolId priority. Changed from multi-role-first (activeRoleId → user.schoolId) to legacy-first (user.schoolId → activeRoleId). This ensures production environments continue working even if activeRoleId backfill migrations haven't run, while still supporting multi-role users who have null user.schoolId. Critical for production deployments where migrations may not execute automatically. Location: server/api/school-admin.ts lines 85-148.
- **CRITICAL FIX: Enrollment Count Display (PRODUCTION-READY)**: Resolved zero enrollment count bug affecting all class views (school admin, educator, parent). Root cause: `getEnrollmentCountForProgram()` used invalid status values ('pending', 'confirmed', 'active') not in schema, and filtered by programId instead of classId. Created new `getEnrollmentCountForClass()` method filtering by classId/marketplaceClassId with correct statuses (pending_payment, enrolled, waitlist, completed). Updated ALL class endpoints to calculate enrollment counts dynamically: school-admin (list/detail), educator (list), public/parent (list/detail/category). Implemented in both database-storage and memory-storage for consistency.
- **Parent Profile Endpoint Migration (PRODUCTION-READY)**: Migrated parent-profile endpoint from Auth0 to Supabase authentication with full multi-role support. Uses active role's school ID for multi-role admins via user_roles table query. Supports all three admin roles (admin, superAdmin, schoolAdmin) with proper tenant isolation. Backward compatible with legacy single-role users.
