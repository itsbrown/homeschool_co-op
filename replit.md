...
-   **Credit Management Admin UI** (Jan 2026): School admin dashboard at `/school-admin/credits` for managing household credits. Features: overview dashboard with stats cards (total available, pending approvals, expiring soon), household balances table, pending approvals workflow with approve/reject actions, transaction history view, and manual credit creation dialog for awarding credits to parents. API: `server/api/credits.ts`.
