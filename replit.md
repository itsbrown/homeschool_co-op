...
-   **Reading Assessment Tracking System**: McCall-Crabbs format with auto-averaging of grade-level scores, automatic Lexile score conversion, and parent/admin reporting. Score format validation enforced on POST and PATCH /api/assessments/students (numeric, fraction, level, percentage, letter_grade). Letter grades normalized to uppercase. `displayScore()` utility in `client/src/lib/assessmentUtils.ts` formats scores for display across all assessment views.
-   **Student Detail Pages (Expanded)**: EducatorStudentDetailPage and StudentDetailPage (school admin) now include tabbed Assessments and Attendance history. Both include a "Print Report" button that triggers a printable ProgressReport component (`client/src/components/student/ProgressReport.tsx`).
-   **Parent Academic Tab**: ChildProfilePage academic tab now populated with real assessment data via `/api/assessments/parent/my-children` (parent role) or `/api/assessments/students/child/:id` (school admin).
-   **Early Departure Attendance Status**: AttendanceTracker now supports `early_departure` status with orange color coding. AttendanceManagementPage status filter includes `early_departure` option.
