/**
 * Production: record Venmo pay-in-full for Verryluz Pagan — Chaska Spring (#426).
 *
 *   $383.12 via Venmo (pay in full)
 *
 *   node scripts/with-prod-env.mjs npx tsx server/scripts/apply-verryluz-venmo-payment-production.ts --dry-run
 *   node scripts/with-prod-env.mjs npx tsx server/scripts/apply-verryluz-venmo-payment-production.ts
 */
...
const PARENT_ID = 67;
const PARENT_EMAIL = 'verryluzpagan@yahoo.com';
const ENROLLMENT_ID = 426;
const SCHEDULED_PAYMENT_ID = 518;
const SCHOOL_ID = 2;
const AMOUNT_CENTS = 38312;
const SYNTHETIC_PI = 'MANUAL-VENMO-verryluz-chaska-2026-06-10';
  ...
  await db.transaction(async (tx) => {
    const [payment] = await tx
      ...
      .values({
        ...
        amount: AMOUNT_CENTS,
        currency: 'usd',
        childName: enrollment.childName ?? 'Chaska Carbonell',
        className: enrollment.className ?? 'Yankee Doodle | Greece',
        description: 'Manual Venmo payment — pay in full (Chaska Spring 2026)',
        status: 'completed',
    ...
    await tx.insert(auditLogs).values({
      ...
      metadata: {
        script: 'apply-verryluz-venmo-payment-production.ts',
        parentId: PARENT_ID,
