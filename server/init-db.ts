...
async function runMigrations() {
  ...
  try {
    ...
    // Application-level duplicate checking is in place in dbStorage.ts
    ...
    // Add volunteer_waiver_id column to school_classes table
    console.log('Running migration: Adding volunteer_waiver_id column to school_classes table...');
    await db.execute(sql`
  ...
  // Add assessment source, lexile, and sessions support
  try {
    const db = await getDb();
    
    console.log('Running migration: Adding assessment source enum and columns...');
    ...
    console.log('✅ Migration completed: assessment source and lexile columns added');
    
    // Create assessment_sessions table
    console.log('Running migration: Creating assessment_sessions table...');
    await db.execute(sql`
    ...
    console.log('✅ Migration completed: assessment_sessions table created');
  } catch (sessionError) {
  ...
  try {
    ...
    `);
    console.log('Running migration: NY IHIP quarterly report tables...');
    await db.execute(sql`
    ...
    `);
    await db.execute(sql`
      CREATE INDEX IF NOT EXISTS idx_quarterly_reports_child ON quarterly_progress_reports(child_id, school_year, quarter);
    `);
  ...
  console.log('Running migration: school analytics event tables...');
  try {
    await db.execute(sql`
    ...
    `);
    console.log('✅ Migration completed: school analytics event tables');
  } catch (analyticsMigrationError: any) {
    console.log('Migration note (non-blocking):', analyticsMigrationError.message);
  }
}
