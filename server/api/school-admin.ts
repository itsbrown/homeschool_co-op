...
router.patch("/schools/:id", async (req, res) => {
  try {
    ...
    console.log(`🔄 Updating school ${schoolId} with data:`, req.body);
    ...
    if (isDevelopment) {
      ...
      if (adminUser) {
        ...
        console.log('✅ Development mode: Using admin user:', user.email);
      }
    } else {
      // Production mode - require valid Supabase authentication
      const { createClient } = await import('@supabase/supabase-js');
    ...
    // Update school in database using storage
    try {
      ...
      return res.status(500).json({ message: "Error updating school" });
    }
  } catch (error) {
    console.error("Error in school update:", error);
    return res.status(500).json({ message: "Error updating school information" });
  }
});
