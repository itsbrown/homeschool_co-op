  ...
  try {
    ...
    try {
      ...
      if (hasMembership && parentUserId && membershipSchoolId) {
        ...
        // Step 1: Generate memberId only if user doesn't have one (separate from enrollment logic)
        ...
        // Step 2: Check for existing membership enrollment (mirrors webhook handler)
        ...
        if (existingEnrollment) {
          if (existingEnrollment.status === 'enrolled') {
            ...
            console.log(`🎫 Membership enrollment ${existingEnrollment.id} already enrolled for user ${parentUserId} - skipping`);
          } else {
            ...
            await storage.updateMembershipEnrollment(existingEnrollment.id, {
              ...
              expirationDate,
              paymentMethod: 'other',
              notes: `Stripe payment via cart checkout (${paymentIntent.id}) - confirm endpoint${membershipDiscountName ? ` - Discount: ${membershipDiscountName}` : ''}`
            });
          ...
          membershipCreated = true;
          console.log(`🎫 ✅ Created new membership enrollment for user ${parentUserId} in confirm endpoint`);
        }
        
        // Step 3: Track discount usage if applicable
        if (membershipCreated && membershipDiscountId && membershipDiscountAmount > 0 && membershipSchoolId) {
          try {
