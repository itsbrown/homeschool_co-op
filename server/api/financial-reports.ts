...
// Data reconciliation endpoint - syncs scheduled payments with actual enrollment balances
router.post('/reconcile-scheduled-payments', async (req: any, res) => {
  try {
    ...
    // Get all enrollments for this school with their balances
    ...
    for (const enrollment of enrollments) {
      // Get all pending scheduled payments for this enrollment
      const pendingPayments = await db
        ...
        .where(
          and(
            eq(scheduledPayments.enrollmentId, enrollment.id),
            eq(scheduledPayments.status, 'pending')
          )
        )
        .orderBy(scheduledPayments.scheduledDate);

      const scheduledTotal = pendingPayments.reduce((sum: number, p: { amount: number }) => sum + p.amount, 0);
      const enrollmentBalance = enrollment.remainingBalance || 0;

      // If scheduled payments total exceeds enrollment balance, mark excess as paid
      if (scheduledTotal > enrollmentBalance) {
        const excessAmount = scheduledTotal - enrollmentBalance;
        let remainingExcess = excessAmount;
        let markedCount = 0;

        // Mark payments as paid starting from the earliest, until we've covered the excess
        for (const payment of pendingPayments) {
          ...
          remainingExcess -= payment.amount;
          markedCount++;
        }
        ...
        reconciliationResults.push({
          ...
          enrollmentBalance: enrollmentBalance / 100,
          scheduledPaymentsTotal: scheduledTotal / 100,
          action: `Marked ${markedCount} payment(s) as paid`,
          paymentsMarkedPaid: markedCount,
        });
      } else if (scheduledTotal < enrollmentBalance && scheduledTotal === 0 && enrollmentBalance > 0) {
        // No scheduled payments but there's a balance - flag for review
        reconciliationResults.push({
          enrollmentId: enrollment.id,
          childName: enrollment.childName || 'Unknown',
          className: enrollment.className || 'Unknown',
          enrollmentBalance: enrollmentBalance / 100,
          scheduledPaymentsTotal: scheduledTotal / 100,
          action: 'No scheduled payments exist for outstanding balance - needs manual review',
          paymentsMarkedPaid: 0,
        });
      }
    }

    const totalMarkedPaid = reconciliationResults.reduce((sum, r) => sum + r.paymentsMarkedPaid, 0);

    res.json({
      success: true,
      message: `Reconciliation complete. ${totalMarkedPaid} payments marked as paid.`,
      results: reconciliationResults,
      summary: {
        enrollmentsProcessed: enrollments.length,
        enrollmentsWithChanges: reconciliationResults.length,
        paymentsMarkedPaid: totalMarkedPaid,
      }
    });

  } catch (error) {
    console.error('Error reconciling scheduled payments:', error);
    res.status(500).json({ error: 'Failed to reconcile scheduled payments' });
  }
});
  ...
  try {
    ...
    for (const enrollment of enrollments) {
      ...
      try {
        ...
        if (parent) parentEmail = parent.email;
      } catch (e) {}
      ...
      if (scheduledTotal !== enrollmentBalance) {
        discrepancies.push({
          ...
          enrollmentBalance: enrollmentBalance / 100,
          scheduledPaymentsTotal: scheduledTotal / 100,
          difference: (scheduledTotal - enrollmentBalance) / 100,
    ...
    res.json({
      ...
      summary: {
        ...
        enrollmentsWithDiscrepancies: discrepancies.length,
        totalOverscheduled: discrepancies.filter(d => d.difference > 0).reduce((sum, d) => sum + d.difference, 0),
      }
    });
