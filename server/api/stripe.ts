...
// Manual enrollment update endpoint (temporary fix while webhook debugging)
router.post('/manual-update-enrollment', async (req, res) => {
  try {
    console.log('🔧 Manual enrollment update triggered');
    
    const { storage } = await import('../storage');
    const { sendPaymentConfirmationEmail } = await import('../lib/email-service');
    
    // Find the pending enrollment for the test child
    const allEnrollments = await storage.getAllEnrollments();
    const pendingEnrollment = allEnrollments.find(e => 
      (e as any).status === 'pending_payment' && 
      (e as any).parentEmail === 'jocimarie@gmail.com' &&
      (e as any).childName === 'Test Child'
    ) as any;
    
    if (!pendingEnrollment) {
      return res.status(404).json({ error: 'No pending enrollment found' });
    }
    
    console.log('📝 Found pending enrollment:', pendingEnrollment.id);
    
    // Update enrollment to enrolled status
    pendingEnrollment.status = 'enrolled';
    pendingEnrollment.amount = 14; // $14 deposit paid
    pendingEnrollment.amountPaid = 14;
    pendingEnrollment.remainingBalance = 126; // $140 - $14 = $126
    pendingEnrollment.paymentIntentId = 'pi_manual_' + Date.now();
    
    await storage.updateEnrollment(pendingEnrollment);
    console.log('✅ Enrollment updated to enrolled status');
    
    // Send payment confirmation email
    const testPayment = {
      id: Date.now(),
      stripePaymentIntentId: pendingEnrollment.paymentIntentId,
    ...
    res.json({ 
      success: true, 
      message: 'Enrollment updated and email sent',
