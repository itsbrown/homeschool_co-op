#!/usr/bin/env node
/**
 * Build-time database initialization script
 * Runs database migrations ONCE during deployment build phase
 * Should NOT run on every server startup
 */

import { createClassesTable } from './classes-db.js';
...
async function runMigrations() {
  try {
    ...
    console.log('🔄 Running build-time database migrations...');
...
async function initializeDatabase() {
  try {
    console.log('🔨 Build-time database initialization...');
    ...
    console.log('✅ Build-time database initialization complete.');
  } catch (error) {
    console.error('❌ Error during build-time initialization:', error);
    console.log('Continuing - application will use runtime fallbacks');
  }
}
