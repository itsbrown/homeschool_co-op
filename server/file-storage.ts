...
export class FileStorage implements IStorage {
  // Define the path for payments data file
  private paymentsFile = path.join(process.cwd(), 'data/payments.json');
  ...
  async createUser(user: InsertUser): Promise<User> {
    ...
    try {
      const users = await this.getUsers(); // Using helper method
      ...
      const newUser: User = {
        ...
        role: user.role || 'parent', // Default role
        name: user.name,
      ...
      users.push(newUser);
      await this.saveUsers(users); // Using helper method
  ...
  private async getUsers(): Promise<User[]> {
    return this.readFile<User>(path.join(process.cwd(), 'data/users.json'));
  }

  private async saveUsers(users: User[]): Promise<void> {
    await this.writeFile<User>(path.join(process.cwd(), 'data/users.json'), users);
  }
  ...
  // Add the getAllPayments method to the file storage implementation
  async getAllPayments(): Promise<Payment[]> {
    try {
      // Use the helper method to read from the payments file
      const payments = await this.readFile<Payment>(this.paymentsFile);
      console.log(`✅ FileStorage retrieved ${payments.length} payments`);
      return payments;
    } catch (error) {
      console.error('Error reading payments file:', error);
      // Return an empty array to prevent crashing if the file is missing or corrupted
      return [];
    }
  }
  ...
  // Existing createPayment method (unchanged as per the provided snippet analysis)
  ...
  // Placeholder for getPayments, assuming it's used by createPayment and should also read from the file.
  // This is inferred to make createPayment functional with the file storage.
  private async getPayments(): Promise<Payment[]> {
    return this.readFile<Payment>(this.paymentsFile);
  }
}