...
function generateUnits(subject: string, gradeLevel: string, learningStyles: string[]): CurriculumTemplate["units"] {
  // This would be generated by Claude in a real implementation
  // For now, we'll create a simplified template based on the subject
...
export function curriculumTemplateToDbFormat(template: CurriculumTemplate, authorId: number): Omit<Curriculum, "id" | "createdAt" | "updatedAt"> {
  return {
    ...
    subject: template.title.split(" ")[0], // Extract subject from title
    gradeLevel: template.title.split("for ")[1], // Extract grade level from title
    authorId: authorId,
    isPublished: false,
    isPublic: false,
    price: 0,
    learningStyles: template.title.toLowerCase().includes("visual") ? ["visual"] : [],
    content: template
  };
}
