...
export async function sendWelcomeEmail(email: string, name: string) {
  ...
  `;
  return sendEmail(email, name, 'Welcome to American Seekers Academy', htmlContent, undefined, 'welcome_service');
}

export async function sendVerificationEmail(email: string, token: string) {
  const verificationUrl = `${process.env.APP_URL || 'https://accounts.americanseekersacademy.com'}/verify-email?token=${token}`;
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background-color: #4F46E5; padding: 24px; text-align: center;">
  ...
  `;
  return sendEmail(email, email, 'Verify Your Email - American Seekers Academy', htmlContent, undefined, 'email_verification');
}
...
export async function sendPasswordResetEmail(email: string, resetUrl: string) {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 24px;">
        ...
        <div style="background-color: #FEF3C7; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #F59E0B;">
          <p style="margin: 0; color: #92400E;"><strong>Click the button below to reset your password:</strong></p>
        </div>
        <div style="text-align: center; margin: 30px 0;">
        ...
        <p style="color: #6B7280; font-size: 14px; margin-top: 20px;">
          Best regards,<br>The American Seekers Academy Team
        </p>
      </div>
  ...
  `;
  return sendEmail(email, email, 'Reset Your Password - American Seekers Academy', htmlContent, undefined, 'password_reset_service');
}
...
export async function sendStaffInvitationEmail(email: string, firstName: string, lastName: string, role: string, department: string, message?: string) {
  ...
  return { messageId: `mock-brevo-staff-${Date.now()}`, response: 'Mock Brevo email service' };
}
...
export async function sendRoleInvitationEmail(email: string, role: string, token: string) {
  ...
  return { messageId: `mock-brevo-role-${Date.now()}`, response: 'Mock Brevo email service' };
}
...
export async function sendEnrollmentReminderEmail(data: EnrollmentReminderData): Promise<boolean> {
  ...
  const cartUrl = data.cartUrl || `${process.env.APP_URL || 'https://accounts.americanseekersacademy.com'}/cart`;
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 24px;">
        ...
        <div style="background-color: #FEF3C7; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #F59E0B;">
          <p style="margin: 0; color: #92400E; font-weight: bold; font-size: 16px;">⚠️ Your spot is not guaranteed until payment is received</p>
        </div>
        <p style="font-size: 15px; color: #374151;">
          You have an enrollment pending payment for <strong>${data.childName}</strong>. Spots are limited and filling up fast.
        </p>
        ...
        <p style="color: #6B7280; font-size: 14px; margin-top: 30px;">
          Best regards,<br>The ${schoolName} Team
        </p>
      </div>
      <div style="background-color: #F3F4F6; padding: 16px; text-align: center; font-size: 12px; color: #6B7280;">
  ...
  const subject = `Action Required: Complete ${data.childName}'s Enrollment for ${data.className}`;
  return sendEmail(data.parentEmail, data.parentName, subject, htmlContent, undefined, 'enrollment_reminder');
}
...
export async function sendBulkEnrollmentReminderEmail(data: BulkEnrollmentReminder): Promise<boolean> {
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 24px;">
        <p style="font-size: 16px;">Dear ${data.parentName},</p>
        <div style="background-color: #FEF3C7; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #F59E0B;">
          <p style="margin: 0; color: #92400E; font-weight: bold; font-size: 16px;">⚠️ Your spots are not guaranteed until payment is received</p>
        </div>
        <p style="font-size: 15px; color: #374151;">
  ...
  const subject = `Action Required: ${data.enrollments.length} Pending Enrollment${data.enrollments.length !== 1 ? 's' : ''} - Complete Payment`;
  return sendEmail(data.parentEmail, data.parentName, subject, htmlContent, undefined, 'bulk_enrollment_reminder');
}
