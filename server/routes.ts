...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Direct activity generation endpoints to bypass Vite middleware
  app.post("/api/activity-gen", async (req, res) => {
    try {
      const { z } = await import('zod');
      const { storage } = await import('./storage');
      const { checkOpenAIStatus, generateEducationalActivity } = await import('./services/openai');
      const backgroundTaskManager = (await import('./services/backgroundTasks')).default;
      const { processKnowledgeBases } = await import('./services/knowledgeBaseExtraction');
      ...
      const params = ActivityGenerationSchema.parse(req.body);
      ...
      backgroundTaskManager.startJob(jobId, async () => {
        console.log(`Applying enhanced semantic processing for subject: ${params.subject}, age range: ${params.ageRange}`);
        
        // Get knowledge base content
        let knowledgeBaseContent = "";
        if (params.knowledgeBaseIds && params.knowledgeBaseIds.length > 0) {
          knowledgeBaseContent = await processKnowledgeBases(params.knowledgeBaseIds, params.subject, params.ageRange);
        }
        ...
        if (generatedActivity && typeof generatedActivity === 'object') {
          const activityData = {
            ...generatedActivity,
          ...
          return {
            success: true,
            activityId: savedActivity?.id || null,
            hasActivity: !!savedActivity,
  ...
  app.get("/api/activity-status/:jobId", async (req, res) => {
    try {
      const backgroundTaskManager = (await import('./services/backgroundTasks')).default;
      ...
      if (jobStatus.status === "completed") {
        ...
        if (jobStatus.result?.activity?.id) {
          ...
          id = jobStatus.result.activityId;
        ...
        message = "Activity generation in progress";
      } else {
        message = "Activity generation pending";
      }
