...
import { z } from "zod";
import { insertCurriculumSchema, insertLessonSchema, insertEventSchema, insertMarketplaceItemSchema, insertKnowledgeBaseSchema, insertChildSchema, insertEmergencyContactSchema, insertProgramSchema, insertProgramEnrollmentSchema, insertMembershipEnrollmentSchema, userRoles, users } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // PUBLIC QR SESSION ENDPOINT (no auth) — teacher clock-in scan page
  app.get("/api/public/session-by-qr/:token", async (req, res) => {
    try {
      ...
      const educator = session.educatorId
        ? await storage.getUser(session.educatorId)
        : null;
