  ...
  try {
    ...
    for (const child of jsonChildren) {
      try {
        ...
        migrationResults.children++;
        console.log(`Migrated child: ${child.firstName} ${child.lastName}`);
      } catch (error) {
        const errorMsg = `Error migrating child ${child.firstName} ${child.lastName}: ${error}`;
        console.error(errorMsg);
        migrationResults.errors.push(errorMsg);
      }
    }
    ...
    for (const school of jsonSchools) {
      try {
        ...
        migrationResults.schools++;
        console.log(`Migrated school: ${school.name}`);
      } catch (error) {
        const errorMsg = `Error migrating school ${school.name}: ${error}`;
        console.error(errorMsg);
        migrationResults.errors.push(errorMsg);
      }
    }
    ...
    for (const classData of jsonClasses) {
      try {
        ...
        migrationResults.classes++;
        console.log(`Migrated class: ${classData.title}`);
      } catch (error) {
        const errorMsg = `Error migrating class ${classData.title}: ${error}`;
        console.error(errorMsg);
        migrationResults.errors.push(errorMsg);
      }
    }
...
router.get('/status', async (req, res) => {
  try {
    ...
    const classCount = await pool.unsafe('SELECT COUNT(*) FROM classes');
    
    res.json({
      success: true,
      tables: {
        users: parseInt(userCount[0].count),
        children: parseInt(childCount[0].count),
        schools: parseInt(schoolCount[0].count),
        classes: parseInt(classCount[0].count)
      }
    });
  } catch (error) {
    res.json({
