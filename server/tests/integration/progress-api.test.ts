...
describeWithDb('Integration: progress API', () => {
  ...
  beforeEach(async () => {
    ...
    educatorEmail = `progress_ed_${uid}@test.com`;
    const educator = await testDb.createTestUser({
      username: `progress_ed_${uid}`,
      email: educatorEmail,
      role: 'educator',
      name: 'Progress Educator',
    });
  ...
  it('GET /subjects returns default seeded subjects', async () => {
    const res = await request(app)
      .get('/api/progress/subjects')
      .set('x-test-user-email', educatorEmail);
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.some((s: { key: string }) => s.key === 'math' || s.key === 'reading')).toBe(true);
  });
});
