...
import { formatJobsSubtitle, holdsParentAndTeaching, holdsSchoolAdmin, holdsTeaching } from "@/lib/user-jobs";
...
import { LogOut, Menu, User, Bell, Home, Users, BookOpen, Calendar, DollarSign, Settings, FolderOpen, Sparkles, GraduationCap, Clock, ClipboardList, Building2, Shield, ChevronDown, LayoutGrid, CalendarDays, ClipboardCheck, UserCheck, AlertTriangle, RefreshCw, X, ShoppingBag, HelpCircle } from "lucide-react";
...
export default function ParentAppShell({ children }: ParentAppShellProps) {
  ...
  const isMultiRoleUser = showTeachingNav || (holdsSchoolAdmin(availableRoles) && availableRoles.length > 1);
  ...
  const [familySectionOpen, setFamilySectionOpen] = useState(!isEducatorRoute);
  ...
  useEffect(() => {
    ...
    if (!isEducatorRoute) setFamilySectionOpen(true);
  ...
  return (
    <LayoutShellProvider>
    <CartProvider>
      <div className="min-h-screen bg-gray-50" data-testid="shell-parent">
        {/* Desktop sidebar */}
                  ...
                  <div className="flex h-full flex-col">
                    ...
                    <ScrollArea className="flex-1 px-3 py-4">
                      <nav className="space-y-1">
                        {showTeachingNav ? (
                          <Collapsible open={familySectionOpen} onOpenChange={setFamilySectionOpen}>
                            <CollapsibleTrigger
                              className={cn(
                        ...
                        {showTeachingNav && (
                          <Collapsible open={educatorSectionOpen} onOpenChange={setEducatorSectionOpen}>
                            <CollapsibleTrigger
                              ...
                              data-testid="nav-group-teaching"
                            >
                              <div className="flex items-center gap-3">
                                ...
                                return (
                                  <SheetClose asChild key={item.href}>
                                    <Link
                                      ...
                                      data-testid={'testId' in item && item.testId ? item.testId : undefined}
                                    >
                                      <Icon className="h-5 w-5" />
                                      <span className="flex-1">{item.title}</span>
                                    </Link>
                    ...
                    <div className="border-t p-4">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">{jobsSubtitle}</p>
                        </div>
                      </div>
