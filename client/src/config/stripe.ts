 ...
 * - Development: Uses TESTING_VITE_STRIPE_PUBLIC_KEY
...
export const getStripePublishableKey = (): string => {
  if (isDevelopment) {
    const testKey = import.meta.env.TESTING_VITE_STRIPE_PUBLIC_KEY;
    if (testKey) {
      console.log('🧪 Using Stripe TEST public key for development');
      return testKey;
    }
    console.warn('⚠️ TESTING_VITE_STRIPE_PUBLIC_KEY not found, falling back to VITE_STRIPE_PUBLIC_KEY');
  }
